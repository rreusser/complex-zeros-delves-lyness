...
var isDirty = true;
var timer = new ResetTimer(500).start();
var E = Complex(1, 0);
var rho = 1.0;
var nu = 0.33;
var h = 0.1;
var w = 7.63;
var cl, ct, w2cl2, w2ct2;
var viscoelasticity = 0;

function computeConstants (omega, viscoelasticity) {
  w = omega;
  E = Complex(Math.cos(viscoelasticity), Math.sin(viscoelasticity));

  cl = E.sqrt().mul(Math.sqrt((1 - nu) / rho / (1 + nu) / (1 - 2 * nu)));
  ct = E.sqrt().div(Math.sqrt(2 * rho * (1 + nu)));

  w2cl2 = cl.pow(-2).mul(w * w);
  w2ct2 = ct.pow(-2).mul(w * w);
}

computeConstants(w, viscoelasticity);

controlPanel([
  {label: 'omega', type: 'range', min: 0.1, max: 10, step: 0.01},
  //{label: 'viscoelasticity', type: 'range', min: 0, max: Math.PI * 2.0}
], {theme: 'dark', position: 'top-left'}).on('input', function (data) {

  computeConstants(data.omega, data.viscoelasticity);
...
regl.frame(function(frameData) {
  camera(function(data) {
    ...
    computeConstants(w, Math.PI * frameData.time);
